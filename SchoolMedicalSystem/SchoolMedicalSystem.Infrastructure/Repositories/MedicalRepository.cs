...
namespace SchoolMedicalSystem.Infrastructure.Repositories
{
    public class MedicalRepository : IMedicalRepository
    {
        ...
        public MedicalRepository(SchoolMedicalDbContext Context) => _context = Context;
        ...
        {
            return await _context.Set<Medicine>().Where(x => x.category == "medicine").ToListAsync();
        }

        public Task<Medicine?> GetByIdAsync(int id)
        {
            return _context.Set<Medicine>().Where(s => s.category == "medicine").FirstOrDefaultAsync(x => x.medicine_id == id);
        }
